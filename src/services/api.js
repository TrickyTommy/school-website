...
const BASE_URL = 'https://smkbudimuliakrw.sch.id';
...
// Helper: baca response sebagai teks lalu parse JSON secara aman.
// Jika body kosong dan status OK → anggap sukses.
// Jika body bukan JSON valid → lempar error dengan info teks aslinya.
const parseResponse = async (response) => {
  ...
  if (!text || !text.trim()) {
    ...
    throw new Error(`Server mengembalikan response kosong (HTTP ${response.status})`);
  }
  try {
